...
import org.derpfest.flash.camera.CameraFacing
import org.derpfest.flash.camera.CameraManager
import org.derpfest.flash.camera.CameraMode
import org.derpfest.flash.camera.CameraState
import org.derpfest.flash.camera.CameraViewModel
import org.derpfest.flash.camera.FlashMode
import org.derpfest.flash.camera.FrameRate
import org.derpfest.flash.camera.VideoStabilizationMode
import org.derpfest.flash.ext.*
import org.derpfest.flash.qr.QrImageAnalyzer
import org.derpfest.flash.ui.CapturePreviewLayout
import org.derpfest.flash.ui.CountDownView
import org.derpfest.flash.ui.GridView
import org.derpfest.flash.ui.HorizontalSlider
import org.derpfest.flash.ui.LensSelectorLayout
import org.derpfest.flash.ui.LevelerView
import org.derpfest.flash.ui.LocationPermissionsDialog
import org.derpfest.flash.ui.PreviewBlurView
import org.derpfest.flash.ui.VerticalSlider
import org.derpfest.flash.utils.AssistantIntent
import org.derpfest.flash.utils.BroadcastUtils
import org.derpfest.flash.utils.CameraSoundsUtils
import org.derpfest.flash.utils.ExifUtils
import org.derpfest.flash.utils.GoogleLensUtils
import org.derpfest.flash.utils.GridMode
import org.derpfest.flash.utils.MediaStoreUtils
import org.derpfest.flash.utils.MediaType
import org.derpfest.flash.utils.PermissionsUtils
import org.derpfest.flash.utils.Rotation
import org.derpfest.flash.utils.ShortcutsUtils
import org.derpfest.flash.utils.StorageUtils
import org.derpfest.flash.utils.TimeUtils
import org.derpfest.flash.utils.TimerMode
...
open class CameraActivity : AppCompatActivity() {
    ...
    private val gestureDetector by lazy {
        GestureDetector(this, object : GestureDetector.SimpleOnGestureListener() {
            ...
            override fun onFling(
                ...
                if (!handler.hasMessages(org.derpfest.flash.CameraActivity.Companion.MSG_ON_PINCH_TO_ZOOM) &&
    ...
    private val scaleGestureDetector by lazy {
        ScaleGestureDetector(this, object : ScaleGestureDetector.SimpleOnScaleGestureListener() {
            override fun onScale(detector: ScaleGestureDetector): Boolean {
                ...
                handler.removeMessages(org.derpfest.flash.CameraActivity.Companion.MSG_ON_PINCH_TO_ZOOM)
                handler.sendMessageDelayed(handler.obtainMessage(org.derpfest.flash.CameraActivity.Companion.MSG_ON_PINCH_TO_ZOOM), 500)
    ...
    private val handler = object : Handler(Looper.getMainLooper()) {
        override fun handleMessage(msg: Message) {
            ...
            when (msg.what) {
                org.derpfest.flash.CameraActivity.Companion.MSG_HIDE_ZOOM_SLIDER -> {
                    zoomLevel.visibility = View.GONE
                }
                org.derpfest.flash.CameraActivity.Companion.MSG_HIDE_FOCUS_RING -> {
                    viewFinderFocus.visibility = View.GONE
                }
                org.derpfest.flash.CameraActivity.Companion.MSG_HIDE_EXPOSURE_SLIDER -> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        cameraController.tapToFocusState.observe(this) {
            when (it) {
                CameraController.TAP_TO_FOCUS_STARTED -> {
                    ...
                    handler.removeMessages(org.derpfest.flash.CameraActivity.Companion.MSG_HIDE_FOCUS_RING)
                    ...
                    handler.sendMessageDelayed(handler.obtainMessage(org.derpfest.flash.CameraActivity.Companion.MSG_HIDE_FOCUS_RING), 500)
                }
            }
        }
        ...
        viewFinder.setOnClickListener { view ->
            ...
            handler.removeMessages(org.derpfest.flash.CameraActivity.Companion.MSG_HIDE_EXPOSURE_SLIDER)
            handler.sendMessageDelayed(handler.obtainMessage(org.derpfest.flash.CameraActivity.Companion.MSG_HIDE_EXPOSURE_SLIDER), 2000)
        ...
        cameraController.zoomState.observe(this) {
            ...
            handler.removeMessages(org.derpfest.flash.CameraActivity.Companion.MSG_HIDE_ZOOM_SLIDER)
            handler.sendMessageDelayed(handler.obtainMessage(org.derpfest.flash.CameraActivity.Companion.MSG_HIDE_ZOOM_SLIDER), 2000)
        ...
        exposureLevel.textFormatter = {
            ...
            if (ev == 0) "0" else org.derpfest.flash.CameraActivity.Companion.EXPOSURE_LEVEL_FORMATTER.format(ev).toString()
        }
        ...
        when (cameraMode) {
            CameraMode.PHOTO -> startShutterAnimation(org.derpfest.flash.CameraActivity.ShutterAnimation.InitPhoto)
            CameraMode.VIDEO -> startShutterAnimation(org.derpfest.flash.CameraActivity.ShutterAnimation.InitVideo)
            else -> {}
        }
        ...
        shutterButton.setOnClickListener {
            // Shutter animation
            when (cameraMode) {
                CameraMode.PHOTO -> startShutterAnimation(org.derpfest.flash.CameraActivity.ShutterAnimation.PhotoCapture)
                CameraMode.VIDEO -> {
                    if (countDownView.cancelCountDown()) {
                        startShutterAnimation(org.derpfest.flash.CameraActivity.ShutterAnimation.VideoEnd)
                        return@setOnClickListener
                    }
                    if (cameraState == CameraState.IDLE) {
                        startShutterAnimation(org.derpfest.flash.CameraActivity.ShutterAnimation.VideoStart)
                    }
                }
                else -> {}
            }
    ...
    private fun startShutterAnimation(shutterAnimation: org.derpfest.flash.CameraActivity.ShutterAnimation) {
        ...
        when (shutterAnimation) {
            org.derpfest.flash.CameraActivity.ShutterAnimation.InitPhoto,
            org.derpfest.flash.CameraActivity.ShutterAnimation.InitVideo -> drawable.reset()
            else -> drawable.start()
        }
    }

    private fun startVideoRecordingStateAnimation(animation: org.derpfest.flash.CameraActivity.VideoRecordingStateAnimation) {
        // Get appropriate drawable
        val drawable = ContextCompat.getDrawable(
        ...
        // Start or reset animation
        when (animation) {
            org.derpfest.flash.CameraActivity.VideoRecordingStateAnimation.Init -> drawable.reset()
            else -> drawable.start()
        }
    }
    ...
    private fun takePhoto() {
        ...
        val photoOutputStream = if (singleCaptureMode) {
            ByteArrayOutputStream(org.derpfest.flash.CameraActivity.Companion.SINGLE_CAPTURE_PHOTO_BUFFER_INITIAL_SIZE_BYTES)
        } else {
            null
        }
        ...
        cameraController.takePicture(
            ...
            object : ImageCapture.OnImageSavedCallback {
                override fun onError(exc: ImageCaptureException) {
                    Log.e(org.derpfest.flash.CameraActivity.Companion.LOG_TAG, "Photo capture failed: ${exc.message}", exc)
                ...
                override fun onImageSaved(output: ImageCapture.OutputFileResults) {
                    ...
                    Log.d(org.derpfest.flash.CameraActivity.Companion.LOG_TAG, "Photo capture succeeded: ${output.savedUri}")
                    cameraState = CameraState.IDLE
                    shutterButton.isEnabled = true
                    if (!singleCaptureMode) {
            ...
            videoRecording = cameraController.startRecording(
                ...
                when (it) {
                    is VideoRecordEvent.Start -> runOnUiThread {
                        ...
                        startVideoRecordingStateAnimation(org.derpfest.flash.CameraActivity.VideoRecordingStateAnimation.Init)
                    }
                    is VideoRecordEvent.Pause -> runOnUiThread {
                        cameraState = CameraState.RECORDING_VIDEO_PAUSED
                        startVideoRecordingStateAnimation(org.derpfest.flash.CameraActivity.VideoRecordingStateAnimation.ResumeToPause)
                    }
                    is VideoRecordEvent.Resume -> runOnUiThread {
                        cameraState = CameraState.RECORDING_VIDEO
                        startVideoRecordingStateAnimation(org.derpfest.flash.CameraActivity.VideoRecordingStateAnimation.PauseToResume)
                    }
                    ...
                    is VideoRecordEvent.Finalize -> {
                        ...
                        if (it.error != VideoRecordEvent.Finalize.ERROR_NO_VALID_DATA) {
                            Log.d(org.derpfest.flash.CameraActivity.Companion.LOG_TAG, "Video capture succeeded: ${it.outputResults.outputUri}")
                            if (!singleCaptureMode) {
        ...
        cameraController.initializationFuture.addListener({
            ...
            cameraController.camera2CameraControl?.apply {
                captureRequestOptions = CaptureRequestOptions.Builder()
                    ...
                    .build()
            } ?: Log.wtf(org.derpfest.flash.CameraActivity.Companion.LOG_TAG, "Camera2CameraControl not available even with camera ready?")
        }, ContextCompat.getMainExecutor(this))
        ...
        when (cameraMode) {
            CameraMode.PHOTO -> {
                if (this.cameraMode == CameraMode.VIDEO) {
                    startShutterAnimation(org.derpfest.flash.CameraActivity.ShutterAnimation.VideoToPhoto)
                } else {
                    startShutterAnimation(org.derpfest.flash.CameraActivity.ShutterAnimation.InitPhoto)
                }
            }
            CameraMode.VIDEO -> {
                if (this.cameraMode == CameraMode.PHOTO) {
                    startShutterAnimation(org.derpfest.flash.CameraActivity.ShutterAnimation.PhotoToVideo)
                } else {
                    startShutterAnimation(org.derpfest.flash.CameraActivity.ShutterAnimation.InitVideo)
                }
            }
            else -> {}
        }
    ...
    private fun sendIntentResultAndExit(input: Any) {
        ...
        outputUri?.let {
            try {
                ...
                Log.e(org.derpfest.flash.CameraActivity.Companion.LOG_TAG, "Failed to open URI")
            ...
            when (input) {
                is InputStream -> {
                    ...
                    val scaledAndRotatedBitmap = bitmap.scale(
                        org.derpfest.flash.CameraActivity.Companion.SINGLE_CAPTURE_INLINE_MAX_SIDE_LEN_PIXELS
                    ).transform(transform)
    ...
    private fun openSettings() {
        val intent = Intent(this, org.derpfest.flash.SettingsActivity::class.java)
        startActivity(intent)
    }
